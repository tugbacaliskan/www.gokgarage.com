/* ============================================================
   GÖK GARAGE OTO GALERİ — script.js  v4.1
   ============================================================ */
...
(function () {
  ...
  if (burger && navLinks) {
    ...
    function calcMenuTop() {
      ...
      return (tb ? tb.offsetHeight : 0) + (nb ? nb.offsetHeight : 62);
    }

    function openMenu() {
      document.documentElement.style.setProperty('--menu-top', calcMenuTop() + 'px');
    ...
    navLinks.querySelectorAll('.nav-item, .nav-wa-mobile').forEach(function (link) {
  ...
  if (document.querySelector('.hero-swiper') && window.Swiper) {
    heroSwiper = new Swiper('.hero-swiper', {
      ...
      on: {
        init: updateHeroCounter,
        realIndexChange: updateHeroCounter,
      },
    });
  }
  ...
  var DEMO_CARS = [
    ...
    { id:'d5', brand:'Porsche',       title:'Panamera Diesel',    variant:'Hatasız · 60.000 KM · Yetkili Bakım · Beyaz',     price:'10.055.000 ₺',km:'60.000 KM',  year:'2015', transmission:'Otomatik', fuel:'Dizel',  status:'active', badge:'ÖZEL', image:'https://i0.shbdn.com/photos/03/16/99/1290031699b15.jpg', url:'https://sahibinden.com/ilan/vasita-otomobil-porsche-gok-garage-porsche-panamera-diesel-hatasiz-60binkm-yetkili-bakim-1290031699/detay' },
  ...
  function renderGrid(cars) {
    if (!grid) return;
    if (!cars || !cars.length) {
      grid.innerHTML = '<div class="vitrin-empty">Gösterilecek araç bulunamadı.</div>';
      return;
    }
    grid.innerHTML = cars.map(function (c) {
      ...
      return (
        '<article class="car-card' + (sold ? ' sold' : '') + '" onclick="window.open(\'' + esc(c.url || '#') + '\',\'_blank\')">' +
          '<div class="car-img">' +
            (c.image
              ? '<img src="' + esc(c.image) + '" alt="' + esc(c.brand + ' ' + c.title) + '" loading="lazy" onerror="this.parentElement.innerHTML=\'<div class=car-img-ph>\u{1F697}</div>\'">'
              : '<div class="car-img-ph">\u{1F697}</div>') +
            (c.badge ? '<span class="car-badge ' + badgeClass(c.badge) + '">' + esc(c.badge) + '</span>' : '') +
          ...
          '<div class="car-body">' +
            ...
            '<div class="car-footer">' +
              ...
              '</div>' +
              '<span class="car-arrow"><i class="bi bi-arrow-right"></i></span>' +
            '</div>' +
          '</div>' +
        '</article>'
      );
  ...
  document.querySelectorAll('.vfilt').forEach(function (btn) {
    btn.addEventListener('click', function () {
      document.querySelectorAll('.vfilt').forEach(function (b) { b.classList.remove('active'); });
  ...
  /* ── 7. SCROLL SPY — aktif menü ────────────────────────── */
  var sections = ['hero', 'vitrin', 'neden', 'reviews', 'sss', 'iletisim'];
  ...
  function onScroll() {
    var scrollY = window.scrollY + 120; // navbar offset
  ...
  onScroll(); // sayfa yüklenince de çalıştır
